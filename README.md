...
**Latest React 19 release:** `19.1.4` for React `19.x`
...
The current stable React 19 release is `19.1.4`. It adds guided structural slots, a headless hook, a state hook, a type-safe factory helper, and a strengthened combobox contract while keeping the styled `<MultiSelectDropdown />` component compatible with the existing visual contract.
...
| **19.x** | **React 19 only** | **`>=19.0.0 <20.0.0`** | **19.1.4 -> 19.2.7** | [React 19 family docs](https://alexandro.net/docs/react/multiselect/react-19/) |
...
Install `19.1.4` for React 19.x applications. The styled component includes its component styles and injects them at runtime. The headless hook does not inject CSS and lets your application own the markup and styling.
...
Version `19.1.4` tightens the interaction details that usually matter most in production forms:
